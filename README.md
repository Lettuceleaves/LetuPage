...
- 收集用户输入
- 发送 HTTP 请求
- 在 bridge ready 后才允许开始交互
- 在 bridge ready 后才允许发送 HTTP
- 在收到简单 `200` 后进入等待状态
...
3. bridge 只回复简单 `200`
4. Agent 读取日志文件
5. Agent 按当前 `README` 流程去修改或新增 page
6. 由于项目运行在 `npm run dev` 下，页面改动会直接呈现给用户
...
- bridge 未 ready 时仍允许开始交互
- bridge 未 ready 时仍允许发送 HTTP
- 在 `vite.config.ts` 中承接业务请求
- 在前端页面代码中直接模拟 Agent 返回结果
- bridge 收到提交后直接生成最终业务结果
