...
如果你的改动影响了以下内容，必须同步更新 [design-doc.md](/E:/projects/LetuPage/design-doc.md) 或 README：
...
README 更偏“Agent 入场说明”，`design-doc.md` 更偏“系统设计文档”。不要把两者写成重复内容。

## 12. 不要做的事

- 不要把复杂业务逻辑直接塞进 `App.vue`
- 不要绕过 `session-manager` 直接手写页面集合状态
- 不要让页面收藏直接依赖 `pageId`
- 不要把具体业务交互硬编码成框架唯一方案
- 不要在没有设计依据时破坏 `main / session / page` 分层
- 不要修改 Git 历史，除非用户明确要求

## 13. 如果你要继续扩展，优先方向

当前最合理的扩展方向通常是：

1. 把 mock HTTP 接口替换成真实后端协议
2. 抽离更多页面类型和页面基类
3. 增加更完整的 Skill 接入点
4. 增加图形资源库和组件资源库的实际目录与索引
5. 增加会话调试面板和协议日志面板

如果需求不明确，先看 [design-doc.md](/E:/projects/LetuPage/design-doc.md)，再决定是扩展实现还是先补设计。
